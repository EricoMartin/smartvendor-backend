...
exports.updateAnalyticsOnSaleCreate = exports.updateAnalyticsOnReceiptCreate = void 0;
...
exports.updateAnalyticsOnSaleCreate = (0, firestore_1.onDocumentCreated)({
    region: "africa-south1",
    ...
    const profit = sale.profit || (sellingPrice - costPrice) * qty;
    const analyticsRef = db.collection("analytics").doc(vendorId);
    const analyticsSnap = await analyticsRef.get();
    // Create analytics if missing
    if (!analyticsSnap.exists) {
    ...
    // Top items
    const topItems = analytics.topItems || [];
    const existing = topItems.find((t) => t.name === itemName);
    if (existing)
        existing.count += qty;
    else
        topItems.push({ name: itemName, count: qty });
    // Total sold
    const totalSold = (analytics.totalSold || 0) + qty;
    const totalProfit = (analytics.totalProfit || 0) + profit;
    // Total revenue
    ...
    const inventorySnap = await db
        ...
        .get();
    const totalAvailable = inventorySnap.docs.reduce((sum, d) => sum + (d.data().stock || 0), 0);
    await analyticsRef.update({
        totalRevenue,
        totalProfit,
        totalSold,
